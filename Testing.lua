...
-- ============================================================
-- CYBER DRAGON v2.1 - AFK CRASH FIX BUNDLE
-- Patched: 2026-06-30
-- Fixes: Memory leaks causing freeze/crash after 5-10 min AFK
-- ============================================================

-- ===== SECTION A: LEAK PREVENTION INFRASTRUCTURE =====
-- Must be injected BEFORE any feature code runs
...
getgenv()._CDLeakFix = {
    ...
    -- Hard limits based on 5-10 min AFK testing
    limits = {
        bulletTracers = 40,
        tracers = 40,
        particles = 60,
    ...
    -- AFK thresholds
    afkThreshold = 30,      -- Enter AFK mode after 30s no input
    cleanupInterval = 10,   -- Cleanup every 10s when AFK
    emergencyInterval = 5,  -- Aggressive cleanup every 5s when critical
}

local _cd = getgenv()._CDLeakFix

-- Safe disconnect helper
function _cd.disconnect(conn)
    if conn and typeof(conn) == "RBXScriptConnection" then
        pcall(function() conn:Disconnect() end)
    end
end

-- Safe remove helper for Drawing objects
function _cd.removeDrawing(obj)
    if obj and typeof(obj) == "Drawing" then
        pcall(function() obj:Remove() end)
    end
end

-- Safe destroy helper
function _cd.destroy(inst)
    if inst and typeof(inst) == "Instance" then
        pcall(function() inst:Destroy() end)
    end
end

-- Connection tracking with auto-dedup
function _cd.trackConnection(category, conn)
    _cd.connections[category] = _cd.connections[category] or {}
    local list = _cd.connections[category]
    -- Remove dead connections first
    for i = #list, 1, -1 do
        if not list[i] or (typeof(list[i]) == "RBXScriptConnection" and not list[i].Connected) then
            table.remove(list, i)
        end
    end
...
-- RenderStep bind deduplication
function _cd.bindRenderStep(name, priority, fn)
    local key = tostring(name)
    if _cd.renderBinds[key] then
        pcall(function() game:GetService("RunService"):UnbindFromRenderStep(key) end)
    end
    pcall(function()
        game:GetService("RunService"):BindToRenderStep(key, priority, fn)
        _cd.renderBinds[key] = true
    end)
end

function _cd.unbindRenderStep(name)
    local key = tostring(name)
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep(key) end)
    _cd.renderBinds[key] = nil
end

-- Drawing object tracking with auto-cleanup
local _origDrawingNew = Drawing.new
Drawing.new = function(className)
    local obj = _origDrawingNew(className)
    table.insert(_cd.drawingObjects, {obj = obj, t = tick()})

    -- Auto-cleanup if over limit
    if #_cd.drawingObjects > _cd.limits.drawingObjects then
        for i = 1, 10 do
            local entry = table.remove(_cd.drawingObjects, 1)
            if entry then _cd.removeDrawing(entry.obj) end
        end
    end
    return obj
end

-- Instance tracking for effects
function _cd.trackEffect(inst)
    if not inst then return end
    table.insert(_cd.effectInstances, {inst = inst, t = tick()})

    -- Auto-cleanup if over limit
    local total = #_cd.effectInstances
    if total > _cd.limits.particles + _cd.limits.billboards + _cd.limits.beams then
        for i = 1, math.min(10, total - _cd.limits.particles) do
            local entry = table.remove(_cd.effectInstances, 1)
            if entry and entry.inst then _cd.destroy(entry.inst) end
        end
    end
end

-- OrigProps tracking with LRU eviction
function _cd.trackOrigProp(part, data)
    if not part then return end
    _cd.origProps[part] = {data = data, t = tick()}

    -- Count and evict if over limit
    local count = 0
    for _ in pairs(_cd.origProps) do count = count + 1 end
    if count > _cd.limits.origProps then
        -- Find oldest entries and remove
        local oldest = {}
        for part, entry in pairs(_cd.origProps) do
            table.insert(oldest, {part = part, t = entry.t})
        end
        table.sort(oldest, function(a, b) return a.t < b.t end)
        for i = 1, math.min(20, count - _cd.limits.origProps) do
            if oldest[i] then _cd.origProps[oldest[i].part] = nil end
        end
    end
end

-- AFK detection
function _cd.updateInput()
    _cd.lastInput = tick()
    if _cd.afkMode then
        _cd.afkMode = false
        print("[CD] AFK mode deactivated - input detected")
    end
end

pcall(function()
    game:GetService("UserInputService").InputBegan:Connect(_cd.updateInput)
end)

-- Emergency cleanup function
function _cd.emergencyCleanup()
    print("[CD] Running emergency memory cleanup...")

    -- 1. Force garbage collection
    collectgarbage("collect")

    -- 2. Clear all tracer arrays aggressively
    if type(bulletTracers) == "table" then
        for i = #bulletTracers, 1, -1 do
            ...
            if tr then
                pcall(function()
                    ...
                    elseif tr.Line then tr.Line:Destroy() end
    ...
    if type(tracers) == "table" then
        for i = #tracers, 1, -1 do
            local tr = tracers[i]
            if tr then
                pcall(function()
                    if tr.IsLine then
                        if tr.Outline then tr.Outline:Remove() end
                        if tr.Line then tr.Line:Remove() end
                    elseif tr.Beam then
                        tr.Beam:Destroy()
                        if tr.Attachment0 then tr.Attachment0:Destroy() end
                        if tr.Attachment1 then tr.Attachment1:Destroy() end
    ...
    -- 3. Clear stale effect instances
    local now = tick()
    for i = #_cd.effectInstances, 1, -1 do
        local entry = _cd.effectInstances[i]
        if entry and (not entry.inst or not entry.inst.Parent or (now - entry.t) > 3) then
            _cd.destroy(entry.inst)
            table.remove(_cd.effectInstances, i)
        end
    end

    -- 4. Clear old drawing objects
    for i = #_cd.drawingObjects, 1, -1 do
        local entry = _cd.drawingObjects[i]
        if entry and (now - entry.t > 5) then
            _cd.removeDrawing(entry.obj)
            table.remove(_cd.drawingObjects, i)
        end
    end

    -- 5. Clear terrain debris
    ...
    print("[CD] Emergency cleanup completed")
end

-- AFK monitor coroutine
task.spawn(function()
    while task.wait(5) do
        local afkTime = tick() - _cd.lastInput

        if afkTime > _cd.afkThreshold then
            _cd.afkMode = true
            local interval = (afkTime > 120) and _cd.emergencyInterval or _cd.cleanupInterval

            if tick() % interval < 5 then
                _cd.emergencyCleanup()
            end
        end

        -- Always do light cleanup every 30s regardless
        if tick() % 30 < 5 then
            collectgarbage("collect")
        end
    end
...
print("[Cyber Dragon] AFK Crash Fix v" .. _cd.version .. " loaded. Memory leak protection active.")

-- ===== SECTION B: RATE LIMITERS =====
-- These prevent spam that causes memory accumulation

-- Silent aim rate limiter
getgenv()._CDSilentRate = {
    lastFire = 0,
    minInterval = 0.08,  -- Max ~12 shots/sec (was unlimited)
    shotsThisSecond = 0,
    secondReset = 0,
    maxPerSecond = 15,   -- Hard cap per second
}

-- ===== SECTION C: HARD CAP OVERRIDES =====
-- These will be injected into the actual functions below
...
getgenv()._CDLeakFix.renderBinds[MENU_CURSOR_BIND] = nil
pcall(function()
...
local function makebullettracer(pos3, endPos, isHit)
    -- FIX: Hard cap to prevent infinite growth
    if type(bulletTracers) == "table" and #bulletTracers >= (getgenv()._CDLeakFix or {}).limits.bulletTracers or 40 then
        local removed = table.remove(bulletTracers, 1)
        if removed then
            pcall(function()
                if removed.Is2D and removed.Segments then
                    for _, seg in ipairs(removed.Segments) do
                        if seg.Line then seg.Line:Remove() end
                    end
                elseif removed.Line then removed.Line:Destroy() end
                if removed.Attachment0 then removed.Attachment0:Destroy() end
                if removed.Attachment1 then removed.Attachment1:Destroy() end
                if removed.Light then removed.Light:Destroy() end
            end)
        end
    end
...
local function firesilent()
    -- FIX: Rate limit to prevent memory spam during AFK
    local now = tick()
    local rs = getgenv()._CDSilentRate
    if now - rs.secondReset >= 1 then
        rs.secondReset = now
        rs.shotsThisSecond = 0
    end
    rs.shotsThisSecond = rs.shotsThisSecond + 1
    if rs.shotsThisSecond > rs.maxPerSecond then return end
    if now - rs.lastFire < rs.minInterval then return end
    rs.lastFire = now
...
local function updaimbot()
    ...
    local ok = pcall(function()
        RunService:UnbindFromRenderStep(AIMBOT_RENDER_BIND)
        getgenv()._CDLeakFix.renderBinds[AIMBOT_RENDER_BIND] = nil
        RunService:BindToRenderStep(AIMBOT_RENDER_BIND, Enum.RenderPriority.Camera.Value + 1, stepAimbot)
        getgenv()._CDLeakFix.renderBinds[AIMBOT_RENDER_BIND] = true
    end)
    if ok then
...
local function spawnHitEmitter(targetPart, config)
    -- FIX: Enforce particle limit
    if getActiveParticles() >= ((getgenv()._CDLeakFix or {}).limits.particles or 60) then
        return nil
    end
    ...
    getgenv()._CDLeakFix.trackEffect(emitter)  -- FIX: Track for cleanup
...
local function fortniteffect(adornee, color, damageText)
    ...
    bb:SetAttribute("_cdCreated", tick())  -- FIX: Age tracking
    getgenv()._CDLeakFix.trackEffect(bb)  -- FIX: Track for cleanup
...
local function saveorigprops(descendant)
    if not origprops[descendant] then
        ...
        getgenv()._CDLeakFix.trackOrigProp(descendant, origprops[descendant])  -- FIX: Track for LRU eviction
...
local function chamloop()
    ...
    local bound = pcall(function()
        RunService:UnbindFromRenderStep(CHAM_RENDER_BIND)
        getgenv()._CDLeakFix.renderBinds[CHAM_RENDER_BIND] = nil
        RunService:BindToRenderStep(CHAM_RENDER_BIND, Enum.RenderPriority.Last.Value, applyChams)
        getgenv()._CDLeakFix.renderBinds[CHAM_RENDER_BIND] = true
    end)
    if not bound then
...
local function maketracer(pos3, endPos)
    -- FIX: Hard cap to prevent infinite growth
    if type(tracers) == "table" and #tracers >= (getgenv()._CDLeakFix or {}).limits.tracers or 40 then
        local removed = table.remove(tracers, 1)
        if removed then
            pcall(function()
                if removed.IsLine then
                    if removed.Outline then removed.Outline:Remove() end
                    if removed.Line then removed.Line:Remove() end
                elseif removed.Beam then
                    removed.Beam:Destroy()
                    if removed.Attachment0 then removed.Attachment0:Destroy() end
                    if removed.Attachment1 then removed.Attachment1:Destroy() end
                end
            end)
...
-- ============================================================
-- CYBER DRAGON - MEMORY LEAK FIX FOOTER
-- Active monitoring and cleanup hooks
-- ============================================================

task.spawn(function()
    while task.wait(8) do
        local _cd = getgenv()._CDLeakFix
        if not _cd then continue end

        -- Light periodic cleanup
        local now = tick()

        -- Clean old drawing objects
        for i = #_cd.drawingObjects, 1, -1 do
            local entry = _cd.drawingObjects[i]
            if entry and (now - entry.t > 10) then
                _cd.removeDrawing(entry.obj)
                table.remove(_cd.drawingObjects, i)
            end
        end

        -- Clean old effect instances
        for i = #_cd.effectInstances, 1, -1 do
            local entry = _cd.effectInstances[i]
            if entry and (not entry.inst or not entry.inst.Parent or (now - entry.t) > 5) then
                _cd.destroy(entry.inst)
                table.remove(_cd.effectInstances, i)
            end
        end
        ...
        -- Clean old origProps
        for part, entry in pairs(_cd.origProps) do
            if not part or not part.Parent or (now - entry.t) > 60 then
                _cd.origProps[part] = nil
            end
        end
...
print("[Cyber Dragon] Memory leak fix footer loaded. Full protection active.")
