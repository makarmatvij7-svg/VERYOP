...
-- ========== UPDATE 20 COMPATIBILITY LAYER ==========
getgenv().CyberDragonUpdate20 = true
getgenv().CyberDragonVersion = "20.0.0"
getgenv().CyberDragonBuildDate = "2026-06-27"

-- Update 20 weapon registry
getgenv().CyberDragonUpdate20Weapons = {
    ["Spear"] = { Type = "Melee", Slot = 3, Range = 12, Damage = 55 },
    ["Grappler"] = { Type = "Melee", Slot = 3, Range = 8, Damage = 45, Special = "Pull" },
    ["Elixir"] = { Type = "Consumable", Slot = 0, Effect = "Heal", Duration = 5 },
    ["RNG Dice"] = { Type = "Consumable", Slot = 0, Effect = "Random", Outcomes = {"Damage", "Heal", "Speed", "Shield"} },
}

-- Update 20: Enhanced safe call wrapper
local function _safeCall20(fn, ...)
    local ok, result = pcall(fn, ...)
    if not ok then
        local errStr = tostring(result):lower()
        if errStr:find("anticheat") or errStr:find("ac_detect") or errStr:find("gameguard") then
            return false, nil
        end
...
getgenv().CyberDragonSafeCall20 = _safeCall20

-- Update 20: Enhanced module loader
local function CyberDragonSafeRequire20(moduleRef, timeoutSec)
    ...
    local lastErr
    local attempts = 0
    while os.clock() < deadline and attempts < 5 do
        attempts = attempts + 1
    ...
    warn("[Cyber Dragon] module load failed (soft): " .. key)
    return nil
end
getgenv().CyberDragonRequire20 = CyberDragonSafeRequire20

-- Update 20: New match state detection
local function detectUpdate20MatchState()
    local lp = game:GetService("Players").LocalPlayer
    if not lp then return false end
    local char = lp.Character
    if char then
        if char:GetAttribute("InMatchV2") == true then return true end
        if char:GetAttribute("Season3Match") == true then return true end
    end
    local rs = game:GetService("ReplicatedStorage")
    local matchData = rs:FindFirstChild("MatchData")
    if matchData then
        local active = matchData:FindFirstChild("Active")
        if active and active.Value == true then return true end
    end
    return false
end
getgenv().CyberDragonDetectMatchV2 = detectUpdate20MatchState
...
local CyberDragonWeaponTranslations = {
    ...
    spanish = {
        ...
        ["Elixir"] = "elixir", ["RNG Dice"] = "dados RNG",
    },
}
...
do
    restricteditems = {
        ...
        "Spear","Grappler","Elixir","RNG Dice"
    }
...
-- ========== UPDATE 20: FINAL INITIALIZATION ==========
task.defer(function()
    repeat task.wait(0.1) until game:IsLoaded()
    repeat task.wait(0.1) until game:GetService("Players").LocalPlayer
    repeat task.wait(0.1) until game:GetService("Players").LocalPlayer:FindFirstChild("PlayerScripts")
    task.wait(1)

    pcall(function()
        if getgenv().CyberDragonApplyAccentTheme then
            getgenv().CyberDragonApplyAccentTheme()
        end
    end)

    pcall(function()
        if getgenv().CyberDragonApplyUiLayering then
            getgenv().CyberDragonApplyUiLayering()
        end
    end)
    ...
    pcall(function()
        if Library and Library.Notify then
            Library:Notify("Cyber Dragon Update 20 Loaded!", 5)
            Library:Notify("New: Spear, Grappler, Elixir, RNG Dice", 5)
        end
    end)
end)

getgenv().CyberDragonCleanup = function()
    pcall(function() if chamLoop then chamLoop:Disconnect() end end)
    pcall(function() if viewportLoop then viewportLoop:Disconnect() end end)
    pcall(function() if tracerconn then tracerconn:Disconnect() end end)
    pcall(function() if aimbotConnection then aimbotConnection:Disconnect() end end)
    pcall(function() RunService:UnbindFromRenderStep("CyberDragonAimbotUpdate") end)
    pcall(function() RunService:UnbindFromRenderStep("CyberDragonMenuCursorDraw") end)
    pcall(function() RunService:UnbindFromRenderStep("CyberDragonViewModelChams") end)
    pcall(function() if fovScreenGui then fovScreenGui:Destroy() end end)
    pcall(function() if Library and Library.Unload then Library:Unload() end end)
end

game:GetService("Players").PlayerRemoving:Connect(function(plr)
    if plr == game:GetService("Players").LocalPlayer then
        pcall(getgenv().CyberDragonCleanup)
    end
end)

getgenv().CyberDragonLoaded = true
getgenv().CyberDragonLoadTime = os.clock()

-- ============================================================
-- END OF CYBER DRAGON UPDATE 20
-- ============================================================
